...
import {
  fetchOpenAiModels,
  OPENAI_MODEL_DISCOVERY_TIMEOUT_MS,
  type OpenAiModelsFetcher,
} from "../openai-model-access.js";
 ...
 */
const PersistedConnectionsSchema = z
  .object({
    version: z.literal(1),
    connections: z.array(
      z
...
const OPENAI_MODEL_ACCESS_TTL_MS = 5 * 60_000;
...
export class ModelSettingsService {
  ...
  private readonly openAiModelsTimeoutMs: number;
  ...
  private openAiModelAccessCache:
    ...
    | undefined;
  private openAiModelAccessRefresh:
    | {
        apiKey: string;
  ...
  private readonly connectionStatuses = new Map<
    LlmProviderId,
    { keyFingerprint: string; status: Exclude<ProviderConnectionStatus, "unknown"> }
  >();
  ...
  constructor(
    ...
    this.providerFactory =
      ...
      ((config, environment, executionProfile) =>
        createProvider(config, environment, fetch, executionProfile ? { executionProfile } : {}));
    ...
    const pricingFetcher =
      options.pricingFetcher === false
        ? undefined
        : (options.pricingFetcher ??
          (options.environment === undefined ? () => fetchOpenRouterPrices() : undefined));
    this.modelPricing = new OpenRouterPricingCatalog(pricingFetcher);
    this.openAiModelsFetcher =
      options.openAiModelsFetcher === false
        ? undefined
        : (options.openAiModelsFetcher ??
          (options.environment === undefined ? fetchOpenAiModels : undefined));
    this.openAiModelsTimeoutMs = options.openAiModelsTimeoutMs ?? OPENAI_MODEL_DISCOVERY_TIMEOUT_MS;
    if (!Number.isFinite(this.openAiModelsTimeoutMs) || this.openAiModelsTimeoutMs <= 0) {
      throw new Error("OpenAI model discovery timeout must be positive");
    }
  ...
  private loadConnectionStatuses(): void {
    try {
      const parsed = PersistedConnectionsSchema.parse(
  ...
  async testProviderConnection(providerId: LlmProviderId): Promise<ProviderConnectionResult> {
    ...
    const definition = SUPPORTED_LLM_PROVIDER_DEFINITIONS.find(
    ...
    if (!apiKey)
      throw new WebApiError(409, `Configure ${definition.envKey} in Settings, or add it to .env`);
  ...
  reloadEnvironment(): void {
    ...
    for (const provider of PUBLIC_LLM_PROVIDER_DEFINITIONS) {
      if (
        previousEnvironment[provider.envKey]?.trim() !== nextEnvironment[provider.envKey]?.trim()
      ) {
  ...
  async testConnections(): Promise<ProviderConnectionResult[]> {
    ...
    const checks = PUBLIC_LLM_PROVIDER_DEFINITIONS.filter((provider) =>
      Boolean(environment[provider.envKey]?.trim()),
    ).map((provider) => ({ provider, apiKey: environment[provider.envKey]!.trim() }));
    const results = await Promise.all(
      checks.map(({ provider, apiKey }) => this.connectionTester(provider.id, apiKey)),
    );
    if (!results.length)
      throw new WebApiError(
        409,
        "Configure at least one provider API key before checking connections",
      );
  ...
  keyStatus(): Record<string, boolean> {
    ...
    return Object.fromEntries(
      SUPPORTED_LLM_PROVIDER_DEFINITIONS.map((provider) => [
        provider.id,
  ...
  isPublicSelection(selection: ModelSelection): boolean {
    return (
      PUBLIC_LLM_PROVIDER_DEFINITIONS.some((provider) => provider.id === selection.provider) &&
  ...
  requirePublicSelection(selection: ModelSelection): void {
    if (!this.isPublicSelection(selection)) {
      throw new WebApiError(
        400,
        `Model ${selection.provider}/${selection.model} is retained for legacy use only`,
      );
    }
  }
  ...
  async openAiModelAccess(): Promise<ReadonlySet<string> | undefined> {
    ...
    if (cached?.apiKey === apiKey) {
      if (cached.expiresAt > Date.now()) return cached.allowedModels;
      void this.refreshOpenAiModelAccess(apiKey);
      return cached.allowedModels;
    }
    if (this.openAiModelAccessRefresh?.apiKey === apiKey) {
      return this.openAiModelAccessRefresh.promise;
    }
    return this.refreshOpenAiModelAccess(apiKey);
  }

  private refreshOpenAiModelAccess(apiKey: string): Promise<ReadonlySet<string> | undefined> {
    if (!this.openAiModelsFetcher) return Promise.resolve(undefined);
    if (this.openAiModelAccessRefresh?.apiKey === apiKey) {
      return this.openAiModelAccessRefresh.promise;
    }
    const fetcher = this.openAiModelsFetcher;
    const promise = (async (): Promise<ReadonlySet<string> | undefined> => {
      let timeout: NodeJS.Timeout | undefined;
      try {
        const allowedModels = await Promise.race([
          fetcher(apiKey),
          new Promise<never>((_resolve, reject) => {
            timeout = setTimeout(
              () => reject(new Error("OpenAI model discovery timed out")),
              this.openAiModelsTimeoutMs,
            );
          }),
        ]);
        const result = new Set(allowedModels);
        if (this.effectiveEnvironment().OPENAI_API_KEY?.trim() === apiKey) {
          this.openAiModelAccessCache = {
            apiKey,
            allowedModels: result,
            expiresAt: Date.now() + OPENAI_MODEL_ACCESS_TTL_MS,
          };
        }
        return result;
      } catch {
    ...
    this.openAiModelAccessRefresh = { apiKey, promise };
    void promise.finally(() => {
      if (this.openAiModelAccessRefresh?.promise === promise) {
        this.openAiModelAccessRefresh = undefined;
      }
    });
    return promise;
  }
              ...
              provider.models.map(async (model) => {
                ...
                return {
                  ...
                  ...(provider.id !== "openai" || openAiModels === undefined
                    ? {}
                    : {
                        keyAccess: openAiModels.has(model.model)
                          ? ("allowed" as const)
