...
export const ModelExecutionProfileKeySchema = z
  .object({
...
const PersistedModelExecutionProfilesSchema = z
  .object({
    ...
    for (const [index, profile] of value.profiles.entries()) {
      ...
      if (keys.has(key)) {
        context.addIssue({
          code: "custom",
          path: ["profiles", index],
          message: "duplicate provider/model/route profile",
        });
      }
      keys.add(key);
      if (profile.fingerprint !== modelExecutionProfileFingerprint(profile)) {
        context.addIssue({
          code: "custom",
          path: ["profiles", index, "fingerprint"],
          message: "profile fingerprint does not match execution settings",
        });
      }
    }
  });
...
function mergeShippedProfiles(
  saved: PersistedModelExecutionProfiles,
): PersistedModelExecutionProfiles {
  const profiles = new Map(
    SHIPPED_MODEL_EXECUTION_PROFILES.map((profile) => [profileKey(profile.key), profile]),
  );
  for (const profile of saved.profiles) {
    const shipped = profiles.get(profileKey(profile.key));
    if (
      !shipped ||
      profile.adapterRevision === MODEL_EXECUTION_ADAPTER_REVISION ||
      shipped.adapterRevision !== MODEL_EXECUTION_ADAPTER_REVISION
    ) {
      profiles.set(profileKey(profile.key), profile);
    }
  }
  return PersistedModelExecutionProfilesSchema.parse({
    ...
    profiles: [...profiles.values()].sort((left, right) =>
      profileKey(left.key).localeCompare(profileKey(right.key)),
    ),
  });
}
...
export class ModelExecutionProfileStore {
  ...
  async get(key: ModelExecutionProfileKey): Promise<FrozenModelExecutionProfile | undefined> {
    ...
    const found = saved.profiles.find(
      (profile) => profileKey(profile.key) === profileKey(parsedKey),
    );
  ...
  async require(key: ModelExecutionProfileKey): Promise<FrozenModelExecutionProfile> {
    ...
    if (!profile) {
      throw new Error(
        `No calibrated execution profile for ${key.provider}/${key.model} via ${key.route}; run playtest calibrate first`,
      );
    }
    ...
    await withSerializedFileLock(this.lockPath, "model execution profile store", async () => {
      ...
      saved.profiles = [
        ...saved.profiles.filter(
          (candidate) => profileKey(candidate.key) !== profileKey(parsed.key),
        ),
        parsed,
  ...
  private async load(): Promise<PersistedModelExecutionProfiles> {
    return mergeShippedProfiles(await this.loadPersisted());
  }

  private async loadPersisted(): Promise<PersistedModelExecutionProfiles> {
    try {
      return PersistedModelExecutionProfilesSchema.parse(
      ...
      if ((error as NodeJS.ErrnoException).code === "ENOENT") {
        return { version: MODEL_EXECUTION_PROFILE_STORE_VERSION, profiles: [] };
      }
      throw error;
    }
  }
