...
import type { CampaignLogSnapshot, CampaignStartSettings, PlayerVisibleTurn } from "./types.js";
...
function renderCompletedStoryMarkdown(snapshot: CampaignLogSnapshot): string | undefined {
  if (!snapshot.completedStory) return undefined;
  const copy = languageDefinition(snapshot.state.language).campaignExport;
  return `## ${copy.shortStory}\n\n${storyText(snapshot.completedStory.story.trim())}`;
}
...
function renderTurnMarkdown(
  ...
  sections.push(
    `#### ${appeal ? copy.playerAppeal : metadataText(playerName)}`,
    quote(entry.action),
  );
...
export function renderCampaignMarkdown(snapshot: CampaignLogSnapshot): string {
  ...
  const sections = [
    ...
    renderCompletedStoryMarkdown(snapshot),
    `## ${copy.turnLog}`,
    body,
  ];
...
function renderTurnHtml(snapshot: CampaignLogSnapshot, entry: PlayerVisibleTurn): string {
  ...
  entries.push(chatEntryHtml("dm", appeal ? copy.decision : copy.dungeonMaster, entry.narration));
  return entries.join("\n");
}
...
function renderCompletedStoryHtml(snapshot: CampaignLogSnapshot): string {
  if (!snapshot.completedStory) return "";
  const copy = languageDefinition(snapshot.state.language).campaignExport;
  return `<section class="completed-story" aria-labelledby="completed-story-title">
  <h2 id="completed-story-title">${escapeHtml(copy.shortStory)}</h2>
  <div class="completed-story-body">${proseHtml(snapshot.completedStory.story)}</div>
</section>`;
}
...
<main>
  ...
  ${renderCompletedStoryHtml(snapshot)}
