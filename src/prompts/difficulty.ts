...
export const CHECK_DIFFICULTY_POLICY: PromptSection = section(
  ...
  `First identify whether the submitted action contains a consequential success/failure proposition. If it does not, use ordinary decision=resolved with no indicator. If it does, use the contract's automatic-success encoding when authoritative state makes the attempted outcome certain, its automatic-failure encoding when the submitted method makes it impossible, or check_required when established opposition or danger leaves meaningful uncertainty.
...
Prefer these anchors. Use an intermediate integer only when the established fiction clearly lies between two anchors. Routine actions use ordinary resolved; consequential but fictionally certain outcomes use an automatic-outcome encoding rather than difficulty 5.
...
An established trait or capability can make an action possible without automatically making it easier. Treat that as access, not a modifier. When a capability materially improves an already possible uncertain attempt, use at most one positive modifier for that capability, label it with the exact concise trait or capability name, and use recent equivalent check calibration when circumstances are materially equivalent. Hard capability limits are never difficulty: use automatic_failure for a consequential attempted effect beyond them rather than offering a high-difficulty check.

Use only circumstances supported by current authoritative state. A newer status, condition, or fact that ends or contradicts an older reputation, advantage, or capability makes the older circumstance unavailable. After modifiers, if success depends only on the natural-100 override because no ordinary roll can reach the difficulty, use automatic_failure. Conversely, if every ordinary roll succeeds and only the natural-1 override can fail, use automatic_success unless an established danger makes that exceptional mishap a meaningful stake; do not use a performative check for an already-certain agreement.
