...
- threadOrdinal: for update_thread and resolve_thread, the thread's 1-based number in the active-thread list in context. Never a thread ID or title, never a number outside the list, and 0 for every other effect.
...
- move_entity: targetId=entity, relatedId=destination location, or a same-turn create_entity hint for a location created now. Emit one for the player character and for every other actor whose containing location changed this turn, and for nobody else. Use the smallest important containing place the narration actually reaches: an actor stopped outside a locked, guarded, or hazardous boundary is not inside the place beyond it.
...
- update_thread: threadOrdinal=the thread's number in context, text=the complete rolling case brief no longer than 1600 characters and without repeating the title, references=the complete related entity ID list or the single value "$unchanged" to keep the current links. Compare the prior summary clause by clause and retain every still-unresolved discovery, suspect, source, place, object, participant, constraint, promise, and current disposition before integrating this turn's progress. References are private retrieval metadata, so retain a central DM-known hidden actor or location needed to resolve the objective without exposing it in the player-facing text. Judge the thread against its own immutable objective, never the latest scene: one event may advance several objectives, and updating one thread is never a substitute for another.
- resolve_thread: threadOrdinal=the thread's number in context, lifecycleCode=1 resolved or 2 failed, text=the closure outcome.
- Emit update_thread or resolve_thread only for a thread this turn actually changed. A thread nobody touched needs no effect; leaving it alone is how it stays unchanged.
...
4. THREADS: every active thread this turn materially changed has its update_thread or resolve_thread and no other thread does; a capsule freshness count above zero requires explicit review, and a distinct remaining problem needs create_thread.
