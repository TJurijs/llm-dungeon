...
describe("d100 mechanics", () => {
    ...
    const result = resolveCheck(
      {
        ...
        difficulty: 50,
        modifiers: [],
        successStakes: "Пройти.",
        failureStakes: "Быть замеченным.",
      },
      60,
    );
  ...
  it("makes natural 1 and 100 critical regardless of total", () => {
    expect(
      resolveCheck({ ...check, difficulty: 5, modifiers: [{ label: "Easy", value: 30 }] }, 1)
        .outcome,
    ).toBe("severe_failure");
    expect(
      resolveCheck({ ...check, difficulty: 95, modifiers: [{ label: "Hard", value: -30 }] }, 100)
        .outcome,
    ).toBe("exceptional_success");
  });
