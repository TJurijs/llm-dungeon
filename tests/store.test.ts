...
function recoveryTurnLog(): string {
  return renderTurnLog(1, {
    ...
    resolved: {
      narration: "The interrupted turn is recovered.",
      turnSummary: "Recovery completed.",
      operations: [],
    },
    provider: "fake",
    model: "fake-model",
...
describe("Markdown state store", () => {
  ...
  it("persists valid world changes and hides secrets from inspection", async () => {
      ...
      resolved: {
        ...
        operations: [
          ...
          },
          {
            ...
            ownerId: "player:hero",
            itemId: "item:sealed-letter",
            quantityDelta: 1,
          },
          {
            type: "add_condition",
            targetId: "player:hero",
            condition: "watched by a hooded stranger",
          },
    ...
    expect(reloaded.entities.get("player:hero")?.conditions).toContain(
      "watched by a hooded stranger",
    );
    ...
    expect(character.inventory).toContainEqual(
      expect.objectContaining({ name: "Sealed Letter", quantity: 1 }),
    );
  ...
  it("turns a new loose item's location into location inventory ownership", async () => {
    ...
    expect(loaded.entities.get("location:crooked-crown")?.inventory).toContainEqual({
      entityId: tool.id,
      quantity: 1,
    });
  });
  ...
  it("rejects an invalid complete commit plan before publishing recovery state", async () => {
    const store = await createTestStore();
    await expect(
      store.commitTurn({
        action: "I wait for a response.",
        resolved: {
          narration: "",
          turnSummary: "No valid narration was produced.",
          operations: [],
        },
        provider: "fake",
        model: "fake-model",
      }),
    ).rejects.toThrow(/missing nonempty Narration/);

    await expect(access(store.pendingPath)).rejects.toThrow();
    expect((await store.load()).manifest.turn).toBe(0);
  });

  it("migrates a legacy loose item to location inventory when the item is next transferred", async () => {
    const store = await createTestStore();
    const initial = await store.load();
    const player = initial.entities.get("player:hero")!;
    const sword = initial.entities.get("item:travel-sword")!;
    const location = initial.entities.get(initial.manifest.currentLocationId)!;
    const playerPath = path.join(store.currentDir, "entities", initial.entityFiles.get(player.id)!);
    const swordPath = path.join(store.currentDir, "entities", initial.entityFiles.get(sword.id)!);
    const locationPath = path.join(
      store.currentDir,
      "entities",
      initial.entityFiles.get(location.id)!,
    );
    await writeFile(
      playerPath,
      renderEntity({
        ...player,
        inventory: player.inventory.filter((entry) => entry.entityId !== sword.id),
      }),
      "utf8",
    );
    await writeFile(swordPath, renderEntity({ ...sword, location: location.id }), "utf8");

    const compatible = await store.load();
    expect(compatible.entities.get(location.id)?.inventory).toContainEqual({
      entityId: sword.id,
      quantity: 1,
    });
    expect(compatible.entities.get(sword.id)?.location).toBeUndefined();

    await store.commitTurn({
      action: "Mara picks up the travel sword.",
      resolved: {
        narration: "Mara picks the travel sword up from the tavern floor.",
        turnSummary: "Mara took possession of the travel sword.",
        operations: [
          {
            type: "transfer_item",
            fromId: location.id,
    ...
    const persistedSword = parseEntity(await readFile(swordPath, "utf8"));
    const persistedLocation = parseEntity(await readFile(locationPath, "utf8"));
    expect(persistedSword.location).toBeUndefined();
    expect(persistedLocation.inventory).not.toContainEqual({
      entityId: sword.id,
      quantity: 1,
    });
    expect((await store.load()).entities.get("npc:mara-venn")?.inventory).toContainEqual({
      entityId: sword.id,
      quantity: 1,
    });
  });

  it("keeps legacy inventory cycles loadable but rejects every new cyclic ownership edge", async () => {
    const store = await createTestStore();
    const initial = await store.load();
    const sword = initial.entities.get("item:travel-sword")!;
    const swordPath = path.join(store.currentDir, "entities", initial.entityFiles.get(sword.id)!);
    const legacyBoxId = "item:legacy-box";
    const legacyBoxPath = path.join(store.currentDir, "entities", entityFilename(legacyBoxId));
    await writeFile(
      swordPath,
      renderEntity({
        ...sword,
        inventory: [
          { entityId: sword.id, quantity: 1 },
          { entityId: legacyBoxId, quantity: 1 },
        ],
      }),
      "utf8",
    );
    await writeFile(
      ...
      renderEntity({
        ...
        tags: ["container"],
        updatedTurn: 0,
        description: "A box persisted by an older application version.",
        traits: [],
        conditions: [],
        inventory: [{ entityId: sword.id, quantity: 1 }],
        facts: [],
        relationships: [],
      }),
      "utf8",
    ...
    expect(legacy.entities.get(sword.id)?.inventory).toContainEqual({
      entityId: sword.id,
      quantity: 1,
    });
    expect(legacy.entities.get(sword.id)?.inventory).toContainEqual({
      entityId: legacyBoxId,
      quantity: 1,
    });
    expect(legacy.entities.get(legacyBoxId)?.inventory).toContainEqual({
      entityId: sword.id,
      quantity: 1,
    });
    await store.commitTurn({
      action: "I catch my breath.",
      resolved: {
        narration: "You catch your breath beside the hearth.",
        turnSummary: "The hero caught their breath.",
        operations: [{ type: "add_condition", targetId: "player:hero", condition: "Rested" }],
      },
      provider: "fake",
      model: "fake-model",
    });
    ...
    await expect(
      store.commitTurn({
        action: "I make two containers own each other.",
        resolved: {
          narration: "The impossible nesting cannot become authoritative.",
          turnSummary: "A new inventory cycle was rejected.",
          operations: [
            {
              type: "create_entity",
              entity: {
    ...
    ).rejects.toThrow(/Inventory ownership contains a cycle/);
    expect((await store.load()).manifest.turn).toBe(1);
  });
  ...
  it("repairs uniquely matching type-compatible references with an omitted namespace", async () => {
    ...
    const maraFact = initial.entities
      .get("npc:mara-venn")!
      .facts.find((fact) => fact.text.includes("owns"))!;
      ...
      resolved: {
        ...
        operations: [
          ...
          { type: "update_thread", threadId: threadSuffix, summary: "Mara revealed a new lead." },
          {
            ...
            targetId: "mara-venn",
            factId: factSuffix,
            replacementText: "Mara still owns the Crooked Crown and now trusts the hero.",
          },
        ],
      },
      provider: "fake",
    ...
    expect(
      loaded.entities
        .get("npc:mara-venn")
        ?.facts.some((fact) => fact.text.includes("now trusts the hero")),
    ).toBe(true);
  });
  ...
  it("rejects duplicate temporary references and unknown references without committing", async () => {
    ...
    await expect(
      unknownStore.commitTurn({
        action: "I pursue a nonexistent lead.",
        resolved: {
          narration: "There is no such lead.",
          turnSummary: "Nothing changed.",
          operations: [
            { type: "update_thread", threadId: "does-not-exist", summary: "Impossible." },
          ],
        },
        provider: "fake",
        model: "fake-model",
  ...
  it("applies movement, relationships, threads, fact replacement, and campaign endings", async () => {
        ...
        operations: [
          {
            type: "create_entity",
            entity: {
              id: "location:tavern-yard",
              kind: "location",
              name: "Tavern Yard",
              status: "open",
              location: "location:crooked-crown",
              tags: ["yard"],
              description: "A muddy yard.",
              establishedFacts: [],
              secrets: [],
              playerKnowledge: [],
            },
          ...
          { type: "move_entity", targetId: "player:hero", locationId: "location:tavern-yard" },
          {
            ...
            targetId: "player:hero",
            summary: "Mara trusts the hero with her livelihood.",
          },
          {
            type: "supersede_fact",
            targetId: "npc:mara-venn",
            factId: maraFact.id,
            replacementFactId: "fact:mara-former-owner",
            replacementText: "Mara formerly owned the Crooked Crown.",
          },
          {
            type: "resolve_thread",
            threadId: northernRoadThreadId,
            outcome: "The disappearances were explained.",
            status: "resolved",
          },
          {
            type: "record_major_event",
            eventId: "event:tavern-changed-hands",
            text: "The Crooked Crown changed hands.",
          },
    ...
    const loaded = await store.load();
    const tavernYard = [...loaded.entities.values()].find(
      (entity) => entity.name === "Tavern Yard",
    );
    ...
    expect(
      loaded.entities
        .get("npc:mara-venn")
        ?.facts.some((fact) => fact.text === "Mara formerly owned the Crooked Crown."),
    ).toBe(true);
    expect(loaded.threads[0]?.status).toBe("resolved");
    expect(
      loaded.chronicle.some((event) => event.text === "The Crooked Crown changed hands."),
    ).toBe(true);
  });
  ...
  it("assigns safe generated IDs instead of trusting model-provided fact, thread, and event IDs", async () => {
        ...
        operations: [
          {
            ...
            text: "A blue mark points north.",
          },
          {
            type: "create_thread",
            threadId: "new lead",
            title: "The Blue Mark",
            summary: "The blue mark points north.",
          },
          {
            type: "record_major_event",
            eventId: "major discovery",
            text: "The hero found the blue mark.",
          },
        ],
      },
      provider: "fake",
    ...
    const loaded = await store.load();
    const fact = loaded.entities
      .get("player:hero")
      ?.facts.find((entry) => entry.text === "A blue mark points north.");
  ...
  it("coalesces duplicate location names and rejects contradictory physical item destinations", async () => {
        ...
        operations: [
          {
            type: "create_entity",
            entity: {
              id: "location:the-crooked-crown",
              kind: "location",
              name: "Crooked Crown",
              status: "open",
              tags: [],
              description: "The same tavern.",
              establishedFacts: [],
              secrets: [],
              playerKnowledge: [],
            },
    ...
    expect(
      [...duplicateLoaded.entities.values()].filter((entity) => entity.kind === "location"),
    ).toHaveLength(1);

    const ownershipStore = await createTestStore();
    await expect(
      ownershipStore.commitTurn({
  ...
  it("repairs an unambiguous near-miss reference to a location created in the same transaction", async () => {
        ...
        operations: [
          {
            type: "create_entity",
            entity: {
              id: "location:oakhaven-streets",
              kind: "location",
              name: "Oakhaven Streets",
              status: "open",
              tags: ["public"],
              description: "The muddy town streets.",
              establishedFacts: [],
              secrets: [],
              playerKnowledge: [],
            },
    ...
    const loaded = await store.load();
    const streets = [...loaded.entities.values()].find(
      (entity) => entity.name === "Oakhaven Streets",
    );
  ...
  it("transfers inventory atomically and normalizes an exact split debit-credit pair", async () => {
    ...
    const loaded = await store.load();
    expect(loaded.entities.get("player:hero")?.inventory).not.toContainEqual({
    ...
    expect(splitLoaded.entities.get("player:hero")?.inventory).not.toContainEqual({
      entityId: "item:travel-sword",
      quantity: 1,
    });
    expect(splitLoaded.entities.get("npc:mara-venn")?.inventory).toContainEqual({
      entityId: "item:travel-sword",
      quantity: 1,
    });
  });

  it("normalizes an exact debit and item move into a conserved drop transfer", async () => {
    const store = await createTestStore();
    const result = await store.commitTurnWithResult({
      action: "I drop my travel sword on the tavern floor.",
      resolved: {
        narration: "You set the travel sword loose on the tavern floor.",
        turnSummary: "The travel sword was left loose in the Crooked Crown.",
        operations: [
          {
            type: "change_inventory",
            ownerId: "player:hero",
    ...
    const dropped = await store.load();
    expect(dropped.entities.get("player:hero")?.inventory).not.toContainEqual({
      entityId: "item:travel-sword",
      quantity: 1,
    });
    expect(dropped.entities.get("location:crooked-crown")?.inventory).toContainEqual({
      entityId: "item:travel-sword",
      quantity: 1,
    });
    expect(dropped.entities.get("item:travel-sword")?.location).toBeUndefined();

    await store.commitTurn({
      action: "Mara picks up the travel sword.",
      resolved: {
        narration: "Mara picks the travel sword up from the floor.",
        turnSummary: "Mara took possession of the travel sword.",
        operations: [
          {
            type: "transfer_item",
            fromId: "location:crooked-crown",
            toId: "npc:mara-venn",
            itemId: "item:travel-sword",
            quantity: 1,
          },
    ...
    expect(recovered.entities.get("location:crooked-crown")?.inventory).not.toContainEqual({
      entityId: "item:travel-sword",
      quantity: 1,
    });
    expect(recovered.entities.get("npc:mara-venn")?.inventory).toContainEqual({
      entityId: "item:travel-sword",
      quantity: 1,
    });
  });

  it("rejects self-owned and cyclic inventories while preserving acyclic item containers", async () => {
    const selfOwnedStore = await createTestStore();
    await expect(
      selfOwnedStore.commitTurn({
        action: "I fold the sword into itself.",
        resolved: {
          narration: "The impossible ownership cannot take hold.",
          turnSummary: "The sword did not contain itself.",
          operations: [
            {
              type: "change_inventory",
              ownerId: "item:travel-sword",
              itemId: "item:travel-sword",
              quantityDelta: 1,
            },
          ],
    ...
    ).rejects.toThrow(/cannot contain itself in inventory/);
    expect((await selfOwnedStore.load()).manifest.turn).toBe(0);

    const cyclicStore = await createTestStore();
    await expect(
      cyclicStore.commitTurn({
        action: "I put each of two boxes inside the other.",
        resolved: {
          narration: "The impossible nesting cannot become authoritative.",
          turnSummary: "The cyclic containers were rejected.",
          operations: [
            {
              type: "create_entity",
              entity: {
            ...
            },
            {
              ...
              entity: {
                ...
                status: "open",
                tags: ["container"],
                description: "Another small wooden box.",
                establishedFacts: [],
                secrets: [],
                playerKnowledge: [],
              },
            ...
            {
              type: "change_inventory",
              ownerId: "item:first-box",
              itemId: "item:second-box",
              quantityDelta: 1,
            },
            {
              type: "change_inventory",
              ownerId: "item:second-box",
              itemId: "item:first-box",
              quantityDelta: 1,
            },
          ],
        },
    ...
    expect((await cyclicStore.load()).manifest.turn).toBe(0);

    const nestedStore = await createTestStore();
    await nestedStore.commitTurn({
      action: "I pack a note into a satchel.",
      resolved: {
        narration: "You place a folded note into a satchel and carry it.",
        turnSummary: "The hero carried a satchel containing a note.",
        operations: [
          {
            type: "create_entity",
            entity: {
              ...
              status: "closed",
              tags: ["container"],
              description: "A leather travel satchel.",
              establishedFacts: [],
              secrets: [],
              playerKnowledge: [],
            },
          },
          {
            ...
            entity: {
              ...
              status: "folded",
              tags: ["message"],
              description: "A folded scrap of paper.",
              establishedFacts: [],
              secrets: [],
              playerKnowledge: [],
            },
          ...
          {
            type: "change_inventory",
            ownerId: "item:satchel",
            itemId: "item:folded-note",
            quantityDelta: 1,
          },
        ],
      },
    ...
    const nested = await nestedStore.load();
    const satchel = [...nested.entities.values()].find(
      (entity) => entity.name === "Travel Satchel",
    )!;
    const note = [...nested.entities.values()].find((entity) => entity.name === "Folded Note")!;
    expect(nested.entities.get("player:hero")?.inventory).toContainEqual({
      entityId: satchel.id,
      quantity: 1,
    });
    expect(satchel.inventory).toContainEqual({ entityId: note.id, quantity: 1 });
  });
  ...
  it("blocks an exact repeated abstract inventory credit from the preceding turn", async () => {
    ...
    const silverId = [...afterPayment.entities.values()].find(
      (entity) => entity.name === "Silver Shillings",
    )!.id;
    const repeatedCredit = {
      type: "change_inventory" as const,
      ownerId: "player:hero",
      itemId: silverId,
      quantityDelta: 5,
    };
    await expect(
      store.commitTurn({
        ...
        resolved: {
          narration: "You pocket the payment.",
          turnSummary: "The payment was pocketed.",
          operations: [repeatedCredit],
        },
        provider: "fake",
        model: "fake-model",
      }),
    ...
    expect(await store.buildContext()).toContain(
        ...
        operations: [
          {
            ...
            entity: {
              ...
              status: "open",
              tags: ["gate"],
              description: "A gate beyond the tavern.",
              establishedFacts: [],
              secrets: [],
              playerKnowledge: [],
            },
          },
          {
            ...
            entity: {
              ...
              tags: ["warden"],
              description: "The distant gate warden.",
              establishedFacts: [],
              secrets: ["The warden secretly marks certain wagons."],
              playerKnowledge: [],
            },
          },
          {
            type: "create_thread",
            threadId: "thread:new",
            title: "The Marked Wagons",
            summary: "Learn why wagons are marked.",
            relatedEntityIds: ["npc:new"],
          },
        ],
      },
      provider: "fake",
        ...
        operations: [
          {
            type: "create_entity",
            entity: {
              id: "location:lantern-ward",
              kind: "location",
              name: "Lantern Ward",
              status: "quiet",
              tags: ["district"],
              description: "A rain-dark district around the tavern.",
              establishedFacts: ["Lantern Ward surrounds the Crooked Crown."],
              secrets: [],
              playerKnowledge: [],
            },
          },
          {
            ...
            locationId: "location:lantern-ward",
          },
        ],
      },
      provider: "fake",
        ...
        operations: [
          {
            ...
            entity: {
              ...
              status: "intact",
              tags: ["token"],
              description: "A small existing brass token.",
              establishedFacts: ["The token already exists."],
              secrets: ["A hidden notch marks its edge."],
              playerKnowledge: [],
            },
          },
          {
