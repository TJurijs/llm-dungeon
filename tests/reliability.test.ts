...
import { capturePendingCommitPreimages } from "../src/persistence/commit.js";
import {
  CURRENT_PENDING_COMMIT_FORMAT_VERSION,
  PendingTurnSchema,
} from "../src/persistence/pending.js";
import {
  entityFilename,
  renderChronicle,
  renderEntity,
...
describe("V1 reliability boundaries", () => {
  it("does not let a current pending commit downgrade by omitting its preimages", () => {
    const parsed = PendingTurnSchema.safeParse({
      kind: "commit",
      formatVersion: CURRENT_PENDING_COMMIT_FORMAT_VERSION,
      campaignId: "campaign:current-format",
    ...
    if (!parsed.success) {
      expect(JSON.stringify(parsed.error.issues)).toContain("preimages");
    }
  });
  ...
  it("rejects moving a carried item into the world without transferring ownership", async () => {
    ...
    ).rejects.toThrow(/cannot use move_entity/);
    ...
    await expect(
      store.commitTurn({
        ...
        resolved: {
          ...
          operations: [
            { type: "resolve_thread", threadId, status: "failed", outcome: "Rewritten." },
          ],
        },
        provider: "fake",
        model: "fake-model",
  ...
  it("rejects renaming a location into an established canonical alias", async () => {
    ...
    expect(loaded.manifest.turn).toBe(0);
    expect(
      [...loaded.entities.values()].filter((entity) => entity.kind === "location"),
    ).toHaveLength(1);
  });
  ...
  it("preserves superseded secret history without exposing it in a public entity view", async () => {
    ...
    const secret = before.entities
      .get("npc:mara-venn")!
      .facts.find((fact) => fact.section === "secrets")!;
    await store.commitTurn({
  ...
  it("validates setup graph ownership and thread references before creating files", () => {
    ...
    doubleLocatedItem.entities.find((entity) => entity.id === "item:travel-sword")!.location =
    ...
    looseInitialItem.player.inventory = [];
    looseInitialItem.entities.find((entity) => entity.id === "item:travel-sword")!.location =
      "location:crooked-crown";
    const normalizedLooseItem = validateInitialSetup(looseInitialItem);
    expect(
      normalizedLooseItem.entities.find((entity) => entity.id === "item:travel-sword")?.location,
    ).toBeUndefined();
    expect(
      normalizedLooseItem.entities.find((entity) => entity.id === "location:crooked-crown")
        ?.inventory,
    ).toContainEqual({ entityId: "item:travel-sword", quantity: 1 });

    const selfNested = structuredClone(setupFixture);
    selfNested.entities.find((entity) => entity.id === "location:crooked-crown")!.location =
      "location:crooked-crown";
    selfNested.entities.find((entity) => entity.id === "item:travel-sword")!.location =
      "location:crooked-crown";
    ...
    const selfOwnedItem = structuredClone(setupFixture);
    selfOwnedItem.entities.find((entity) => entity.id === "item:travel-sword")!.inventory = [
      { entityId: "item:travel-sword", quantity: 1 },
    ];
    expect(() => validateInitialSetup(selfOwnedItem)).toThrow(/cannot contain itself in inventory/);

    const cyclicInventory = structuredClone(setupFixture);
    cyclicInventory.entities.push({
      id: "item:sword-sheath",
      kind: "item",
      name: "Sword Sheath",
      status: "intact",
      tags: ["container"],
      description: "A leather sword sheath.",
      establishedFacts: [],
      secrets: [],
      playerKnowledge: [],
      traits: [],
    ...
    cyclicInventory.entities.find((entity) => entity.id === "item:travel-sword")!.inventory = [
      { entityId: "item:sword-sheath", quantity: 1 },
    ];
  ...
  it("allocates distinct persisted fact IDs for entity IDs with the same slug", async () => {
    ...
    const factIds = [...(await store.load()).entities.values()].flatMap((entity) =>
  ...
  it("rejects traversal in a prepared commit before writing outside the campaign", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({
        ...
        writes: {
          "entities/player@hero.md":
            "would corrupt an authoritative file if validation were incremental",
  ...
  it("rejects projected durable state that is not produced by the turn operation ledger", async () => {
    const store = await createTestStore();
    const loaded = await store.load();
    const targetTurn = loaded.manifest.turn + 1;
    const targetTurnPath = `turns/${String(targetTurn).padStart(6, "0")}.md`;
    ...
    const writes = {
      [targetTurnPath]: renderTurnLog(targetTurn, {
        action: "Recovery binding fixture.",
        resolved: {
          narration: "The narration claims an injury.",
          turnSummary: "An unsupported injury was projected.",
          operations: [],
        },
        provider: "fake",
        model: "fake-model",
    ...
    await expect(store.recoverCommit()).rejects.toThrow(
      /operation ledger does not produce its projected entities/,
    );
    const after = await store.readManifest();
    expect(after.turn).toBe(0);
    expect(await readFile(path.join(store.currentDir, playerPath), "utf8")).not.toContain(
      "status: injured",
    );
    expect(await readFile(store.pendingPath, "utf8")).toContain('"kind":"commit"');
  });

  it("replays a bound operation ledger after non-manifest writes were interrupted", async () => {
    const store = await createTestStore();
    const loaded = await store.load();
    const targetTurn = loaded.manifest.turn + 1;
    const targetTurnPath = `turns/${String(targetTurn).padStart(6, "0")}.md`;
    ...
    const writes = {
      [targetTurnPath]: renderTurnLog(targetTurn, {
        action: "I endure the impact.",
        resolved: {
          narration: "The impact leaves a deep bruise.",
          turnSummary: "The hero was bruised.",
      ...
      [playerPath]: renderEntity({
        ...player,
        conditions: [...player.conditions, operation.condition],
        updatedTurn: targetTurn,
      }),
      "manifest.json": `${JSON.stringify(
        {
    ...
    await writeFile(path.join(store.currentDir, targetTurnPath), writes[targetTurnPath], "utf8");
    await writeFile(path.join(store.currentDir, playerPath), writes[playerPath], "utf8");
  ...
  it("rejects malformed or inconsistent planned documents without any filesystem mutation", async () => {
    const corruptions: Array<{
      ...
      },
      {
        ...
        writes: (loaded) => {
          const player = {
            ...loaded.entities.get("player:hero")!,
      ...
      },
      {
        ...
        writes: (loaded) => ({
          "threads.md": renderThreads(
            loaded.threads.map((thread, index) =>
              index === 0 ? { ...thread, relatedEntityIds: ["npc:missing"] } : thread,
            ),
          ),
        }),
      },
      {
        ...
        writes: () => ({
          "chronicle.md": renderChronicle([
            { id: "event:future", text: "Not committed yet.", turn: 2 },
          ]),
        }),
      },
      {
        ...
        writes: (loaded) => {
          ...
          return {
            [`turns/${String(targetTurn).padStart(6, "0")}.md`]: renderTurnLog(targetTurn, {
              ...
              resolved: {
                narration: "Invalid check.",
                turnSummary: "Invalid check.",
                operations: [],
              },
              provider: "fake",
              model: "fake-model",
  ...
  it("does not clear a manifest-committed recovery record until every planned write exists", async () => {
    ...
    const prepared = await writePreparedCommitFixture(store, loaded, {
      [newEntityPath]: newEntity,
    });
    await writeFile(
      path.join(store.currentDir, prepared.targetTurnPath),
      prepared.targetTurnText,
      "utf8",
    );
    await writeFile(
      path.join(store.currentDir, "manifest.json"),
      prepared.targetManifestText,
      "utf8",
    );
  ...
  it("rejects a planned entity that erases an established relationship without mutating files", async () => {
    ...
    await writePreparedCommitFixture(store, loaded, {
      [`entities/${loaded.entityFiles.get(player.id)!}`]: renderEntity({
        ...player,
        relationships: [],
      }),
    });
  ...
  it("rejects a new pending commit for a terminal campaign without mutating files", async () => {
    ...
    await expect(store.recoverCommit()).rejects.toThrow(
      /terminal campaign cannot prepare another turn/,
    );
    expect(await snapshotFiles(store.currentDir)).toEqual(before);
  });
  ...
  it("does not clear a manifest-committed recovery record when a planned write differs", async () => {
    const store = await createTestStore();
    const loaded = await store.load();
    const plannedThreads = renderThreads(
      loaded.threads.map((thread, index) =>
        index === 0 ? { ...thread, summary: "A planned recovered summary." } : thread,
      ),
    );
    const prepared = await writePreparedCommitFixture(store, loaded, {
      "threads.md": plannedThreads,
    });
    await writeFile(
      path.join(store.currentDir, prepared.targetTurnPath),
      prepared.targetTurnText,
    ...
    expect(await readFile(store.pendingPath, "utf8")).toContain('"kind":"commit"');
    expect(await readFile(path.join(store.currentDir, "threads.md"), "utf8")).not.toBe(
      plannedThreads,
    );
  });
  ...
  it("keeps the active campaign when replacement setup validation fails", async () => {
    ...
    await expect(
      store.replaceGame({ setup: invalid, worldRules: "Classic fantasy." }),
    ).rejects.toThrow();
  ...
  it("recovers an accepted campaign replacement after the durable intent is written", async () => {
    ...
    const replacement = await stagedStore.createGame({
    ...
    const archivedManifest = JSON.parse(
      await readFile(path.join(store.archiveDir, archivedDirectory, "manifest.json"), "utf8"),
