...
describe("playtest failure attribution", () => {
  it("attributes account, route, adapter, and candidate failures independently", () => {
    expect(
      attributePlaytestFailure(new GenerationFailure("provider", "forbidden", false, 403), {
        lane: "candidate",
      }),
    ).toMatchObject({ owner: "account_access", candidateStatusImpact: "inconclusive" });
    expect(
      attributePlaytestFailure(new GenerationFailure("rate_limit", "busy", true, 429), {
        lane: "candidate",
      }),
    ...
    expect(
      attributePlaytestFailure(new GenerationFailure("malformed_json", "bad output", true), {
        lane: "candidate",
  ...
  it("keeps truncation, judge, player, and application ownership out of candidate scoring", () => {
    expect(
      attributePlaytestFailure(new GenerationFailure("malformed_json", "truncated", true), {
        lane: "candidate",
        attemptMetadata: { ...metadata, truncated: true },
      }).owner,
    ).toBe("adapter_configuration");
    expect(
      attributePlaytestFailure(new GenerationFailure("wire_schema_violation", "bad judge", true), {
        lane: "judge",
      }),
    ).toMatchObject({ owner: "judge", candidateStatusImpact: "excluded" });
    expect(
      attributePlaytestFailure(new Error("judge provider failed"), {
        lane: "judge",
        stage: "provider_call",
      }),
    ).toMatchObject({ owner: "judge", candidateStatusImpact: "excluded" });
    expect(
      attributePlaytestFailure(new GenerationFailure("malformed_json", "bad player", true), {
        lane: "player_driver",
      }),
    ).toMatchObject({ owner: "player_driver", candidateStatusImpact: "excluded" });
    expect(
      attributePlaytestFailure(new Error("disk"), {
        lane: "candidate",
        stage: "persistence",
      }),
