...
describe("versioned playtest packages", () => {
  ...
  it("preserves all ten behavior profiles for autoplay and stress use", () => {
    ...
    expect(CAMPAIGN_AUTOPLAY_PACKAGE.playerProfiles).toEqual(
      PLAYER_PROFILES.map((profile) => profile.id),
    );
  ...
  it("ships one bilingual canonical certification state that passes production setup validation", () => {
    for (const language of ["en", "ru"] as const) {
      ...
      expect(
        setup.entities.find((entity) => entity.id === "npc:mara-venn")?.inventory,
      ).toContainEqual({ entityId: "item:moonleaf-tonic", quantity: 2 });
      expect(
        setup.entities.find((entity) => entity.id === "location:old-sluice")?.inventory,
      ).toContainEqual({ entityId: "item:brass-gate-key", quantity: 1 });
      expect(
        setup.entities.find((entity) => entity.id === "location:old-sluice")?.inventory,
      ).toContainEqual({ entityId: "item:customs-ledger", quantity: 1 });
      const inventoryItemIds = new Set(
        [...setup.player.inventory, ...setup.entities.flatMap((entity) => entity.inventory)].map(
          (entry) => entry.entityId,
        ),
      );
      expect(
        setup.entities
          .filter((entity) => inventoryItemIds.has(entity.id))
          .every((entity) => entity.location === undefined),
      ).toBe(true);
      expect(
        setup.entities.find((entity) => entity.id === "npc:serik-vale")?.secrets.length,
      ).toBeGreaterThan(0);
      expect(setup.entities.find((entity) => entity.id === "location:old-sluice")?.status).toMatch(
        /warning|сигнальн/i,
      );
  ...
  it("freezes bilingual world and DM style into every canonical package and its hash", () => {
    ...
    for (const playtestPackage of [CERTIFICATION_PACKAGE, TUNING_PACKAGE]) {
      ...
      if (changed.startingState.kind !== "canonical")
        throw new Error("expected canonical package copy");
  ...
  it("keeps model autoplay scalable, resumable in intent, and distinct from profiles", () => {
    ...
    expect(PERSISTENCE_SOAK_PACKAGE.checkpointInjections?.[0]?.coverageRequirementIds).toEqual([
      "early-recall",
    ]);
    expect(
      ADVERSARIAL_BOUNDARIES_PACKAGE.checkpointInjections?.flatMap(
        (item) => item.coverageRequirementIds,
      ),
    ).toEqual(["sandbox-resistance", "secret-safety", "action-economy"]);
    expect(
      MECHANICS_PACKAGE.checkpointInjections?.flatMap((item) => item.coverageRequirementIds),
