...
describe("OpenAI model access discovery", () => {
  it("lists model IDs with the provided key without returning the raw response", async () => {
    ...
    await expect(fetchOpenAiModels("project-secret")).resolves.toEqual(
      new Set(["gpt-5.6-luna", "gpt-5.6-sol"]),
    );
  ...
  it("reports only the HTTP status when discovery is rejected", async () => {
    vi.stubGlobal(
      ...
      vi.fn().mockResolvedValue(
        new Response(JSON.stringify({ error: { message: "sensitive provider response" } }), {
          status: 401,
        }),
      ),
    );

    await expect(fetchOpenAiModels("project-secret")).rejects.toThrow(
      "OpenAI model discovery failed (401)",
    );
  });
});
