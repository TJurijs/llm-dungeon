...
describe("campaign catalog", () => {
  ...
  it("queues overlapping in-process catalog access across instances", async () => {
    ...
    await expect(
      Promise.all([firstCatalog.listCampaigns(), secondCatalog.listCampaigns()]),
    ).resolves.toEqual([
      expect.arrayContaining([
        expect.objectContaining({ title: "Concurrent 1" }),
        expect.objectContaining({ title: "Concurrent 2" }),
      ]),
      expect.arrayContaining([
        expect.objectContaining({ title: "Concurrent 1" }),
        expect.objectContaining({ title: "Concurrent 2" }),
  ...
  it("clears a creation intent after validating an already-promoted campaign", async () => {
    ...
    const manifestBeforeRecovery = await readFile(
    ...
    expect(campaigns).toEqual([expect.objectContaining({ campaignId, title: "Already Promoted" })]);
    expect(await readFile(path.join(store.currentDir, "manifest.json"), "utf8")).toBe(
      manifestBeforeRecovery,
    );
    await expect(access(path.join(dataRoot, CAMPAIGN_CREATION_INTENT_FILE))).rejects.toThrow();
  });
  ...
  it("promotes a complete legacy creation stage without regenerating it", async () => {
    ...
    const stagedManifest = await readFile(
      path.join(stagingStore.currentDir, "manifest.json"),
      "utf8",
    );
    ...
    expect(await readFile(path.join(scopeRoot, "current", "manifest.json"), "utf8")).toBe(
      stagedManifest,
    );
  ...
  it("fails closed when catalog metadata duplicates a creation request", async () => {
    ...
    const first = await catalog.createCampaign({
      setup: setup("First Claim"),
      worldRules: "Rules.",
    });
    const second = await catalog.createCampaign({
      setup: setup("Second Claim"),
      worldRules: "Rules.",
    });
    ...
    for (const campaignId of [first.campaignId, second.campaignId]) {
      const scopeRoot = campaignScopePath(dataRoot, campaignId);
      const metadata = CampaignMetadataSchema.parse(
        JSON.parse(await readFile(path.join(scopeRoot, CAMPAIGN_METADATA_FILE), "utf8")),
      );
      await writeCampaignMetadata(
        scopeRoot,
        CampaignMetadataSchema.parse({
    ...
    await expect(catalog.findCampaignByCreationRequest(requestId)).rejects.toThrow(
      /Multiple campaigns claim/,
    );
  });
  ...
  it("persists provider selection and makes archival irreversible for cached and reopened stores", async () => {
    ...
    await expect(catalog.updateProviderConfig(created.campaignId, openRouter)).rejects.toThrow(
      /unfinished request/,
    );
    ...
    expect((await catalog.archiveCampaign(created.campaignId)).archivedAt).toBe(
      archived.archivedAt,
    );

    await expect(catalog.openCampaign(created.campaignId)).rejects.toThrow(
      /archived and cannot be resumed/,
    );
    await expect(catalog.updateProviderConfig(created.campaignId, gemini)).rejects.toThrow(
      /cannot be changed/,
    );
  ...
  it("refuses to permanently delete an active campaign", async () => {
    ...
    const created = await catalog.createCampaign({
      setup: setup("Still Active"),
      worldRules: "Rules.",
    });

    await expect(catalog.deleteArchivedCampaign(created.campaignId)).rejects.toThrow(
      /must be archived/,
    );
    expect((await catalog.listCampaigns()).map((campaign) => campaign.campaignId)).toContain(
      created.campaignId,
    );
  });
  ...
  it("renames an active campaign atomically and keeps archived campaigns read-only", async () => {
    ...
    expect(
      (await catalog.listCampaigns()).find((campaign) => campaign.campaignId === created.campaignId)
        ?.title,
    ).toBe("A Better Name");
    expect((await created.store.readManifest()).title).toBe("A Better Name");

    await catalog.archiveCampaign(created.campaignId);
    await expect(catalog.renameCampaign(created.campaignId, "Forbidden")).rejects.toThrow(
      /archived and cannot be renamed/,
    );
    expect(
      (await catalog.readCampaign(created.campaignId).then((store) => store.readManifest())).title,
    ).toBe("A Better Name");
  });
  ...
  it("migrates the legacy current save and archives without resurrecting archived campaigns", async () => {
    ...
    const archivedManifest = await readFile(
      path.join(archivedStore.currentDir, "manifest.json"),
      "utf8",
    );
    ...
    expect(campaigns).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          campaignId: archived.campaignId,
          archived: true,
          providerConfig: gemini,
        }),
      ]),
    ...
    expect(await readFile(path.join(currentStore.currentDir, "manifest.json"), "utf8")).toBe(
      currentManifest,
    );
    expect(await readFile(path.join(oldStore.currentDir, "manifest.json"), "utf8")).toBe(
      archivedManifest,
    );
  ...
  it("finishes a migration interrupted after the campaign directory move", async () => {
    ...
    const catalog = new CampaignCatalog(dataRoot);
    expect(await catalog.listCampaigns()).toContainEqual(
      expect.objectContaining({
        campaignId: state.campaignId,
        archived: false,
      }),
  ...
  it("fails closed when a catalog directory manifest has another campaign identity", async () => {
    ...
    await writeFile(
      manifestPath,
      `${JSON.stringify({ ...manifest, campaignId: "campaign:another" }, null, 2)}\n`,
      "utf8",
    );
  ...
  it("rejects conflicting durable migration metadata instead of overwriting it", async () => {
    ...
    await expect(new CampaignCatalog(dataRoot).listCampaigns()).rejects.toThrow(
      /metadata conflicts/,
    );
    expect(
      JSON.parse(await readFile(path.join(scopeRoot, CAMPAIGN_METADATA_FILE), "utf8")),
