...
export function chooseCampaignId(campaigns, preferredId) {
  ...
  return (
    available.find((campaign) => !campaign?.archived)?.campaignId ??
...
export function normalizeTerminalEntry(value, fallbackChannel = "game") {
  ...
  const appealTargetTurn =
    Number.isSafeInteger(value.appealTargetTurn) && value.appealTargetTurn >= 1
  ...
  const generation =
    ...
    typeof sourceGeneration.provider === "string" &&
    sourceGeneration.provider.trim() &&
    typeof sourceGeneration.model === "string" &&
    sourceGeneration.model.trim()
      ? {
...
function withLegacyGameTurnMetadata(entry) {
  ...
  if (
    ["CAMPAIGN BEGINS — ", "КАМПАНИЯ НАЧИНАЕТСЯ — ", "НАЧАЛО КАМПАНИИ — "].some((prefix) =>
  ...
  const match =
    entry.title.match(/^DUNGEON MASTER — TURN ([1-9]\d*)$/) ??
...
function isLegacyEvaluationTranscriptEntry(entry) {
  return (
    entry.title.startsWith("TRANSCRIPT — ") &&
...
export function migrateTerminalEntries(values) {
  ...
  return values
    .map((value) => {
      if (isTerminalChannel(value?.channel))
        return withLegacyGameTurnMetadata(normalizeTerminalEntry(value));
      ...
      if (autoRunHeader || evaluationArtifact) {
        ...
        evaluationOpening &&
        (title.endsWith(" — OPENING") || title.endsWith(" — НАЧАЛО"))
      ) {
        channel = "evaluations";
        evaluationOpening = false;
...
export function parseTerminalHistory(raw) {
  try {
    ...
    const visibleEntries = entries.filter(
      (entry) => !isLegacyEvaluationTranscriptEntry(entry) && !isLegacyInspectionEntry(entry),
    );
    return {
...
function isPlayerEntry(entry) {
  return (
    entry?.channel === "game" && !entry.kind && PLAYER_ACTION_TITLES.has(normalizedTitle(entry))
  );
}
...
function sameProjectedEntry(authoritative, existing) {
  ...
  return (
    authoritative?.channel === existing?.channel &&
...
export function mergeAuthoritativeTerminalEntries(authoritativeValues, existingValues) {
  ...
  for (
    let authoritativeIndex = authoritative.length - 1;
