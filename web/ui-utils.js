      ...
      return models
        .map((model) => {
          ...
          const testedLanguages = Array.isArray(model.testedLanguages)
            ...
            : Array.isArray(model.test?.testedLanguages)
              ? model.test.testedLanguages
              : [];
          const failedLanguages = Array.isArray(model.failedLanguages)
            ? model.failedLanguages
            : Array.isArray(model.test?.failedLanguages)
              ? model.test.failedLanguages
              : [];
          return {
            ...
            available:
              model.available === undefined ? status === "compatible" : Boolean(model.available),
      ...
      (entry) =>
        ...
        (entry.available &&
          ...
          (!language ||
            ...
            entry.testedLanguages.includes(language))),
    );
}
