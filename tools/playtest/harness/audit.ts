...
import type { CoverageRequirement, PlaytestPackage, PlaytestTurnRecord } from "./contracts.js";
...
export function buildMechanicalAudit(
  turns: readonly PlaytestTurnRecord[],
): PlaytestMechanicalAudit {
  ...
  return {
    ...
    assignedRolls: Object.fromEntries(
      turns.map((turn) => [String(turn.turn), turn.assignedNaturalRoll]),
    ),
    operationCounts,
    itemFlows,
    movements,
  ...
  switch (rule.kind) {
    case "check_policy": {
      if (!turn || turn.status !== "completed")
        return { passed: false, evidence: `turn ${rule.turn} did not complete` };
      const checked = turn.check !== undefined;
      const passed =
        rule.policy === "context_dependent" || (rule.policy === "required" ? checked : !checked);
      return {
        passed,
        evidence: `turn ${rule.turn} check=${checked ? "present" : "absent"}; expected ${rule.policy}`,
      };
    }
    case "natural_roll": {
      ...
      return {
        passed,
        evidence: `turn ${rule.turn} assigned=${turn?.assignedNaturalRoll ?? "missing"}, used=${actual ?? "missing"}, expected=${rule.roll}`,
      };
    }
    ...
    case "transfer_item": {
      ...
      return {
        passed: transfer !== undefined,
        evidence: transfer
          ? `turn ${rule.turn} contains the conserved transfer`
          : `turn ${rule.turn} is missing the conserved transfer`,
      };
    }
    case "advance_time": {
      const minutes = operations.reduce(
        (total, operation) => total + (operation.type === "advance_time" ? operation.minutes : 0),
        0,
      );
      return {
        passed: minutes >= rule.minimumMinutes,
        evidence: `turn ${rule.turn} advances ${minutes} minutes`,
      };
    }
    case "move_entity": {
      const move = operations.some(
        (operation) =>
      ...
      return {
        passed: move,
        evidence: move
          ? `turn ${rule.turn} contains the required movement`
          : `turn ${rule.turn} is missing the required movement`,
      };
    }
    case "fact_section": {
      const fact = operations.some(
        (operation) =>
      ...
      return {
        passed: fact,
        evidence: fact
          ? `turn ${rule.turn} persists ${rule.section} on ${rule.targetId}`
          : `turn ${rule.turn} lacks required ${rule.section} fact`,
      };
    }
    case "relationship_update": {
      const relationship = operations.some(
        (operation) =>
      ...
      return {
        ...
        evidence: relationship
          ? `turn ${rule.turn} updates the relationship`
          : `turn ${rule.turn} lacks the relationship update`,
      };
    }
    case "thread_transition": {
      const transition = operations.some(
        (operation) =>
      ...
      return {
        passed: transition,
        evidence: transition
          ? `turn ${rule.turn} transitions ${rule.threadId} to ${rule.status}`
          : `turn ${rule.turn} lacks the required thread transition`,
      };
    }
    case "context_compaction": {
      const observation = turn?.contextObservation;
      if (!observation)
        return { passed: false, evidence: `turn ${rule.turn} has no context observation` };
      const narrationCompacted = !observation.fullNarrationTurns.includes(
        rule.excludedFullNarrationTurn,
      );
      const durablePresent = rule.requiredDurableEntityIds.every((id) =>
        observation.durableEntityIds.includes(id),
      );
      return {
    ...
    case "invariants": {
      const relevant = [...turnsByNumber.values()].filter(
        (candidate) => candidate.turn <= rule.throughTurn,
      );
      const passed =
        relevant.length > 0 &&
        relevant.every(
          (candidate) => candidate.status === "completed" && candidate.invariantStatus === "passed",
        );
      return {
        passed,
        evidence: `${relevant.filter((candidate) => candidate.invariantStatus === "passed").length}/${relevant.length} turns passed invariant checks`,
      };
    }
  }
}
...
export function assessCoverage(
  ...
  const entries: CoverageAssessmentEntry[] = playtestPackage.coverageRequirements.map(
    (requirement) => {
      const requiredTurn =
        requirement.mode === "judge"
          ? requirement.turn
          : "turn" in requirement.rule
            ? requirement.rule.turn
            : requirement.rule.throughTurn;
      if (
        requiredTurn !== undefined &&
