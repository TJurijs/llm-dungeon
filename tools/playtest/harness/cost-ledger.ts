...
export async function reservePlaytestCallCost(
  ...
  await appendPlaytestJsonLine(
    ledgerPath,
    PlaytestCostLedgerEventSchema.parse({
      schemaVersion: 1,
      event: "reserved",
...
export async function unsettledPlaytestCallCost(
  ...
  const recorded = new Set(
    calls.flatMap((call) => (call.costReservationId ? [call.costReservationId] : [])),
  );
  return [...reservations.values()].reduce(
    (sum, reservation) =>
