...
export const PLAYER_PROFILES = [
  {
    id: "curious-explorer",
    instruction: "Explore unfamiliar places, examine details, and follow discoveries organically.",
  },
  {
    id: "social-manipulator",
    instruction:
      "Pursue goals through conversation, bargains, deception, alliances, and NPC relationships.",
  },
  {
    id: "cautious-investigator",
    instruction: "Gather evidence, verify claims, prepare carefully, and revisit unresolved clues.",
  },
  {
    id: "reckless-adventurer",
    instruction: "Take bold risks, escalate tense situations, and accept dangerous consequences.",
  },
  {
    id: "combat-focused",
    instruction:
      "Use force when fictionally reasonable while still adapting to injuries and consequences.",
  },
  {
    id: "creative-problem-solver",
    instruction:
      "Combine ordinary objects, environmental details, and social leverage in unusual ways.",
  },
  {
    id: "rule-challenger",
    instruction:
      "Occasionally attempt impossible actions or claim an unowned item to test whether the world resists unsupported assertions.",
  },
  {
    id: "long-term-planner",
    instruction:
      "Form a durable goal, track promises, and revisit people or places affected by earlier choices.",
  },
  {
    id: "chaotic",
    instruction:
      "Act adversarially and sometimes incoherently: test unsupported possessions, contradictions, malformed input, impossible actions, and recovery after a valid action.",
  },
  {
    id: "in-character",
    instruction:
      "Play strictly as the character your own traits, competencies, temperament, and durable facts establish; let goals and tactics grow from who this character is rather than from any external playstyle archetype.",
  },
...
export const SimulatedPlayerActionSchema = z
  .object({
...
export const ScriptBranchConditionSchema = z.discriminatedUnion("kind", [
  ...
  z
    .object({
      ...
      outcomes: z
        .array(z.enum(["exceptional_success", "success", "failure", "severe_failure"]))
        .min(1),
    })
    .strict(),
]);

export const ScriptedTurnSchema = z
  .object({
    turn: z.number().int().positive(),
  ...
  z
    .object({
      ...
      operationType: StateOperationSchema.options.map((option) => option.shape.type.value).length
        ? z.enum(
            StateOperationSchema.options.map((option) => option.shape.type.value) as [
...
export const PlaytestPackageSchema = z
  .object({
    id: z.string().regex(/^[a-z][a-z0-9-]*$/),
  ...
  .strict()
  .superRefine((playtestPackage, context) => {
    ...
    if (
      ...
      !playtestPackage.checkpointInjections?.length
    ) {
      context.addIssue({
        code: "custom",
    ...
    for (const [index, turn] of (playtestPackage.scriptedTurns ?? []).entries()) {
      if (turn.turn !== index + 1) {
        context.addIssue({
          code: "custom",
          path: ["scriptedTurns", index, "turn"],
          message: "scripted turns must be consecutive from 1",
        });
      }
...
export const PlaytestPackageReferenceSchema = z
  .object({
    id: z.string().regex(/^[a-z][a-z0-9-]*$/),
    version: z.number().int().positive(),
  })
...
export const PlaytestModelCostSchema = z
  .object({
...
export const PlaytestModelTargetSchema = z
  .object({
...
export const PlaytestRunConfigSchema = z
  .object({
    ...
    scenarioSeed: z
      .string()
      .trim()
      .regex(/^[a-z][a-z0-9-]*$/, "scenario seed id must be lowercase kebab-case")
      .optional(),
    tuningVariable: z
      .string()
      .trim()
      .max(500)
      .regex(
        /^(model|adapter|prompt):\s*\S.*$/u,
        "must declare model:, adapter:, or prompt: followed by one variable",
      )
      .optional(),
      ...
      .superRefine((judge, context) => {
        if (judge.policy !== "none" && !judge.target) {
          context.addIssue({
            code: "custom",
            path: ["target"],
            message: "judged runs require a separate judge target",
          });
        }
        if (judge.policy === "none" && judge.target) {
          context.addIssue({
            code: "custom",
            path: ["target"],
            message: "unjudged runs cannot configure a judge target",
          });
        }
        if (judge.policy === "checkpoints_and_final" && !judge.checkpointEvery) {
          context.addIssue({
            ...
            message: "checkpoint judging requires an interval",
          });
        }
        if (judge.policy !== "checkpoints_and_final" && judge.checkpointEvery !== undefined) {
          context.addIssue({
            code: "custom",
            path: ["checkpointEvery"],
            message: "only checkpoint judging accepts an interval",
          });
        }
      }),
  })
...
export const PlaytestRunIdSchema = z
  .string()
  .regex(/^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/, "must be a safe playtest run ID");

export const PlaytestManifestSchema = z
  .object({
  ...
  .superRefine((call, context) => {
    if (!call.success && !call.failureOwner) {
      context.addIssue({
        code: "custom",
        path: ["failureOwner"],
        message: "every failed call requires an owner",
      });
    }
...
export const PlaytestTurnRecordSchema = z
  .object({
    turn: z.number().int().positive(),
    fixtureId: z
      .string()
